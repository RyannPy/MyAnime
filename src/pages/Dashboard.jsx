...
const statsData = [
  ...
  },
  {
    ...
    title: "Anime Terbaru",
    value: "3",
    description: "Rilis terbaru yang sedang trending.",
    icon: (
      <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="mb-8 flex flex-col gap-6">
          ...
          <section>
            <div className="grid gap-6 xl:grid-cols-3">
              {statsData.map((item) => (
                <StatsCard key={item.id} title={item.title} value={item.value} description={item.description} icon={item.icon} />
              ))}
            </div>
          </section>
        </div>
        ...
        <section className="space-y-6">
          ...
          <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
            {dummyAnimes.map((anime) => (
              <AnimeCard key={anime.id} title={anime.title} rating={anime.rating} review={anime.review} image_url={anime.image_url} />
            ))}
          </div>
        </section>
      </main>
