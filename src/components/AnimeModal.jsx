...
const AnimeModal = ({ anime, isOpen, onClose, onEdit, onDeleted }) => {
  ...
  const handleDelete = async () => {
    const ok = window.confirm("Hapus anime ini? Tindakan ini tidak dapat dibatalkan.");
    if (!ok) return;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="mx-4 max-w-4xl rounded-2xl bg-white p-6 shadow-lg">
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          <div>
            <h3 className="text-2xl font-semibold text-slate-900">{anime.title}</h3>
        ...
        <div className="mt-6">
          ...
          <p className="mt-2 text-sm text-slate-600 whitespace-pre-line">{anime.review}</p>
        </div>
